...
SS_solve= function(data,burnin_iter_vec_thin,iter_vec_thin,MCMCstart,g.prior){
  ...
  for (j in 1:len_iter){
    ...
    K_new = (olditer*K_old + iter*sample_ss$K_hat)/(newiter)
  ...
  return(list(g.prior=g.prior,edge_vec=edge_vec,time_vec=time_vec,iter_vec_thin=iter_vec_thin,burnin_iter_vec_thin=burnin_iter_vec_thin,plinks=plinks_new,plinks_iter_matrix=plinks_iter_matrix,K_iter_matrix=K_iter_matrix))
}
